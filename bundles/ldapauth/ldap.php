...
class LDAPauth extends \Laravel\Auth\Drivers\Driver {
	...
	protected function get_account($account, $basedn)
	{
		...
		$attr = array('dn', 'givenname', 'sn', 'cn', 'memberof', 'objectguid', 'mail', 'samaccountname', 'title');
		//$attr = array();
	 ...
	 * Searches the LDAP tree for the specified account or group
	 */
	public function search_ldap($account, $basedn)
	{
		if (is_null($this->conn))
		{
			throw new Exception('No LDAP connection bound');
		...
		$attr = array('dn', 'givenname', 'sn', 'cn', 'memberof', 'objectguid', 'mail', 'samaccountname', 'title');
		//$attr = array();
		$result = @ldap_search($this->conn, $basedn, "(name={$account}*)", $attr);
		if ($result === false)
		{
			return null;
		}
		...
		if ($entries['count'] > 0)
		{    
            $targetCount = (int) round($entries['count'] / 2);
            ...
            {            
                ...
                if ( !isset($value['cn'][0]) || !isset($value['givenname'][0]) )
                {
                    break;
                }
                ...
                // Append results to our response array
                $response[] = $return;
